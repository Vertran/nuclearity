...
class ObjectDrawable2D:
    ...
    def load_texture(self, path):
        try:
            ...
            log.error("An Error occured while loading the texture:", str(e.with_traceback))
...
class RegularPolygon(ObjectDrawable2D):
    def __init__(self, **kwargs):
        ...
        for i in range(self.segments):
            ...
            verts.extend([self.x, self.y, 0.0,
                        0.0, 0.0])
            verts.extend([self.x + self.radius * math.cos(theta1), self.y + self.radius * math.sin(theta1), 0.0,
                        1.0, 0.0])
            verts.extend([self.x + self.radius * math.cos(theta2), self.y + self.radius * math.sin(theta2), 0.0,
                        1.0, 1.0])
...
#           X               Y               Z       U    V      Border
        self.vertices = [
            self.x - hw,    self.y - hh,    0.0,    0.0, 0.0,   1.0,
            self.x + hw,    self.y - hh,    0.0,    1.0, 0.0,   1.0,
            self.x + hw,    self.y + hh,    0.0,    1.0, 1.0,   1.0,

            self.x - hw,    self.y - hh,    0.0,    0.0, 0.0,   1.0,
            self.x + hw,    self.y + hh,    0.0,    1.0, 1.0,   1.0,
            self.x - hw,    self.y + hh,    0.0,    0.0, 1.0,   1.0,
        ]
...
class Button(ObjectDrawable2D):
    def __init__(self, **kwargs): # pos=(0,0), dim=(200, 100), color=(1.0, 0.6, 0.1), thicickness=3, out_color=(1.0, 1.0, 1.0), centered=[False, False],
        super().__init__()
...
#           X                      Y                      Z       U    V      Border
        self.vertices = [
            self.x - hw,           self.y - hh,           0.0,    0.0, 0.0,   0.0,
            self.x + hw,           self.y - hh,           0.0,    1.0, 0.0,   0.0,
            self.x + hw,           self.y + hh,           0.0,    1.0, 1.0,   0.0,

            self.x - hw,           self.y - hh,           0.0,    0.0, 0.0,   0.0,
            self.x + hw,           self.y + hh,           0.0,    1.0, 1.0,   0.0,
            self.x - hw,           self.y + hh,           0.0,    0.0, 1.0,   0.0,

            self.x - hw + thic,    self.y - hh + thic,    0.0,    0.0, 0.0,   1.0,
            self.x + hw - thic,    self.y - hh + thic,    0.0,    1.0, 0.0,   1.0,
            self.x + hw - thic,    self.y + hh - thic,    0.0,    1.0, 1.0,   1.0,

            self.x - hw + thic,    self.y - hh + thic,    0.0,    0.0, 0.0,   1.0,
            self.x + hw - thic,    self.y + hh - thic,    0.0,    1.0, 1.0,   1.0,
            self.x - hw + thic,    self.y + hh - thic,    0.0,    0.0, 1.0,   1.0,
        ]
